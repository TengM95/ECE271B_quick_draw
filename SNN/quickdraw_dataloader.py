...
class QuickDrawDataset(Dataset):
    ...
    def __init__(self, root_dir ,file_dir ,transform=transforms.ToTensor(), color='L'):
        ...
        self.file = np.load(self.file_dir)
    ...
    def __getitem__(self, ind):
        ...
        - A tuple (image, label)
        """
        files = np.load(self.file_dir)
        #print(files.shape)
        pair = files[ind]
        #print(pair)
        pos_0 = pair[:784].reshape((28,28,1))
        pos_1 = pair[784:1568].reshape((28,28,1))
        neg = pair[1568:].reshape((28,28,1))
        
        #If a transform is specified, apply it
        if self.transform is not None:
            pos_0 = self.transform(pos_0)
